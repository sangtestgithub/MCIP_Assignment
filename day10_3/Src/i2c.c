...
void I2CInit() {
    ...
    // setup I2C pins
    RCC->AHB1ENR |= RCC_AHB1ENR_GPIOBEN;
    GPIOB->MODER &= ~(3U << I2C1_SCL*2); // PB6// as output pin 10// earlier value erase
    GPIOB->MODER |=  (2 << I2C1_SCL*2); // AF// value overwrite
    GPIOB->OTYPER |= (1 << I2C1_SCL);   // open-drain
    GPIOB->PUPDR &= ~(3U << I2C1_SCL*2); // no pull-up/down
    GPIOB->MODER &= ~(3U << I2C1_SDA*2); // PB7//as output pin 10 // earlier value erase
    GPIOB->MODER |=  (2 << I2C1_SDA*2); // AF // value overwrite
    ...
    GPIOB->AFR[0] |= (4 << GPIO_AFRL_AFSEL6_Pos); // for pin 6 lower pins of AFR registers
    GPIOB->AFR[0] |= (4 << GPIO_AFRL_AFSEL7_Pos); // for pin 7 lower pins
